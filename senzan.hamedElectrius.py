...
darkblue = '\033[34m' 
white = '\033[00m'
print('\033[31m'+pyfiglet.figlet_format(" hamed Electrius",font='slant')+ "\n "+'\033[34m'+"_"*67+white)

bot = Robot_Rubika("orlrsbrhtxbkqvldezhcdkpuvlfxcfvb")

your_Group_Link="https://rubika.ir/joing/DFIEIDHJ0LNRFLKZNKMLCJAVMRUDQOQY"
...
bot.sendMessage(Group_Guid,"""Sinzan Lion BoT

Made by Hamed Electrios
‌
      !start جهت فعال سازی
‌
‌ID @RoFreack
               https://rubika.ir/joinc/BCDFHHAA0ELHHYTCYKPVFMVELURSZBQI  """)
while 1:
	try:
											...
											for banner in bot.getMessagesInfo(Baner_Guid, [Bener__id]):
												if banner['message_id']==Bener__id:
													if int(teedad_seen) > int(banner['count_seen']) :
														...
														if int(teedad_seen) > int(banner['count_seen']):
															open("Group_Link.txt","w").write("hamed Electrius")
															tedad_link=0
...
لطفا یک عدد از 0 بیشتر انتخاب کنید!""",message_id=msg.get("message_id"))
							...
							try:
								bot.sendMessage(Group_Guid,f"""به بخش دستورات لیون بوت
...
جوین🌚  [@hamed_Electrius]

Lion.bot""",message_id=msg.get("message_id"))
							except:pass
